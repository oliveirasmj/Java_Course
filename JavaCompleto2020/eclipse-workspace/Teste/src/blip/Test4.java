            ...
            if (finalWord.length() == 0) { // se ainda for vazio coloca K e N (as duas logo em primeiro - I e N)
                ...
                if (finalWord.contains(value)) { // se letra ja existir na palavra
                    ...
                    if(finalWord.contains(key)){
                        int posLetra = finalWord.indexOf(key); // R
                        StringBuilder stringBuilder = new StringBuilder(finalWord);
                        stringBuilder.deleteCharAt(posLetra);
                ...
                else if (!finalWord.contains(value)){ // se letra não existir na palavra

                    if (finalWord.contains(key)) { // se key existir na palavra
                        int posLetra = finalWord.indexOf(key); // E
                        // colocar a key depois de value
                        StringBuilder stringBuilder = new StringBuilder(finalWord);
                        stringBuilder.insert(posLetra + 1, value);
                        finalWord = stringBuilder.toString();
                    }
                    else{
                        // colocar a key no final da palavra
                        StringBuilder stringBuilder = new StringBuilder(finalWord);
                        stringBuilder.insert(finalWord.length(), value);
                        finalWord = stringBuilder.toString();
                    }
                }
            }
